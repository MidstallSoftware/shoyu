...
static GParamSpec* shoyu_surface_props[N_PROPERTIES] = { NULL, };
...
static void shoyu_surface_commit(struct wl_listener* listener, void* data) {
  ...
  struct wlr_xdg_toplevel* wlr_xdg_toplevel = wlr_xdg_toplevel_try_from_wlr_surface(self->wlr_surface);
  if (wlr_xdg_toplevel != NULL) {
    if (wlr_xdg_toplevel->base->initial_commit) {
      ShoyuOutput* output = shoyu_compositor_get_xdg_toplevel_claimed_output(self->compositor, wlr_xdg_toplevel);
      if (output != NULL) {
        wlr_xdg_toplevel_set_size(wlr_xdg_toplevel, output->wlr_output->width, output->wlr_output->height);
      } else {
...
static void shoyu_surface_set_property(GObject* object, guint prop_id, const GValue* value, GParamSpec* pspec) {
  ShoyuSurface* self = SHOYU_SURFACE(object);
...
static void shoyu_surface_get_property(GObject* object, guint prop_id, GValue* value, GParamSpec* pspec) {
  ShoyuSurface* self = SHOYU_SURFACE(object);
...
static void shoyu_surface_class_init(ShoyuSurfaceClass* class) {
  ...
  shoyu_surface_props[PROP_COMPOSITOR] = g_param_spec_object(
      ...
      "The compositor the surface comes from.",
      SHOYU_TYPE_COMPOSITOR, G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY);

  g_object_class_install_properties(object_class, N_PROPERTIES, shoyu_surface_props);

  /**
   * ShoyuSurface::destroy:
   * @surface: a #ShoyuSurface
   */
  shoyu_surface_sigs[SIG_DESTROY] = g_signal_new(
      "destroy", SHOYU_TYPE_SURFACE, G_SIGNAL_RUN_LAST,
   ...
   */
  shoyu_surface_sigs[SIG_REALIZED] = g_signal_new(
      "realized", SHOYU_TYPE_SURFACE, G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(ShoyuSurfaceClass, realized), NULL, NULL, NULL, G_TYPE_NONE, 1, G_TYPE_POINTER);

  /**
   * ShoyuSurface::unrealized:
   * @surface: a #ShoyuSurface
   */
  shoyu_surface_sigs[SIG_UNREALIZED] = g_signal_new(
      "unrealized", SHOYU_TYPE_SURFACE, G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(ShoyuSurfaceClass, unrealized), NULL, NULL, NULL, G_TYPE_NONE, 0);

  /**
   * ShoyuSurface::commit:
   * @surface: a #ShoyuSurface
   */
  shoyu_surface_sigs[SIG_COMMIT] = g_signal_new(
      "commit", SHOYU_TYPE_SURFACE, G_SIGNAL_RUN_LAST,
      0, NULL, NULL, NULL, G_TYPE_NONE, 0);
   ...
   * @surface: a #ShoyuSurface
   */
  shoyu_surface_sigs[SIG_MAP] = g_signal_new(
      "map", SHOYU_TYPE_SURFACE, G_SIGNAL_RUN_LAST,
      0, NULL, NULL, NULL, G_TYPE_NONE, 0);
}
...
ShoyuSurface* shoyu_surface_new(ShoyuCompositor* compositor) {
  return SHOYU_SURFACE(g_object_new(SHOYU_TYPE_SURFACE, "compositor", compositor, NULL));
}
