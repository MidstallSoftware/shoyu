...
static GParamSpec* shoyu_shell_props[N_PROPERTIES] = { NULL, };
...
static void shoyu_shell_bind(struct wl_client* wl_client, void* data, uint32_t version, uint32_t id) {
  ShoyuShell* self = SHOYU_SHELL(data);

  g_debug("wl_client#%p wants to bind to ShoyuShell#%p", wl_client, self);

  struct wl_resource* resource = wl_resource_create(wl_client, &shoyu_shell_interface, version, id);

  if (self->resource != NULL) {
    wl_resource_post_error(resource, WL_DISPLAY_ERROR_INVALID_OBJECT, "Only a single client can bind to the Shoyu Shell protocol");
    return;
  }

  wl_resource_set_implementation(resource, &shoyu_shell_impl, self, shoyu_shell_resource_destroy);
...
static void shoyu_shell_constructed(GObject* object) {
  ...
  self->global = wl_global_create(self->compositor->wl_display, &shoyu_shell_interface, 1, g_object_ref(self), shoyu_shell_bind);
  g_assert(self->global);

  self->display_destroy.notify = shoyu_shell_display_destroy;
  wl_display_add_destroy_listener(self->compositor->wl_display, &self->display_destroy);
}
...
static void shoyu_shell_set_property(GObject* object, guint prop_id, const GValue* value, GParamSpec* pspec) {
  ShoyuShell* self = SHOYU_SHELL(object);
...
static void shoyu_shell_get_property(GObject* object, guint prop_id, GValue* value, GParamSpec* pspec) {
  ShoyuShell* self = SHOYU_SHELL(object);
...
static void shoyu_shell_class_init(ShoyuShellClass* class) {
  ...
  shoyu_shell_props[PROP_COMPOSITOR] = g_param_spec_object(
      ...
      "The compositor the shell comes from.",
      SHOYU_TYPE_COMPOSITOR, G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY);

  g_object_class_install_properties(object_class, N_PROPERTIES, shoyu_shell_props);
}
...
ShoyuShell* shoyu_shell_new(ShoyuCompositor* compositor) {
  return SHOYU_SHELL(g_object_new(SHOYU_TYPE_SHELL, "compositor", compositor, NULL));
}
