...
static void shoyu_shell_output_set_surface(struct wl_client* wl_client, struct wl_resource* resource, struct wl_resource* surface) {
  ShellOutput* self = wl_resource_get_user_data(resource);
  struct wlr_surface* wlr_surface = wlr_surface_from_resource(surface);

  ShoyuOutput* output = shoyu_compositor_get_output(self->shell->compositor, self->wlr_output);
...
void shoyu_shell_get_output(struct wl_client* wl_client, struct wl_resource* shell_resource, uint32_t id, struct wl_resource* output) {
  ShoyuShell* shell = wl_resource_get_user_data(shell_resource);
  struct wlr_output* wlr_output = wlr_output_from_resource(output);

  if (wlr_addon_find(&wlr_output->addons, shell, &shoyu_shell_output_addon_impl) != NULL) {
    wl_resource_post_error(shell_resource, SHOYU_SHELL_ERROR_OUTPUT_ALREADY_CONSTRUCTED, "shoyu_shell_output already constructed for this output");
    return;
  }
  ...
  shell_output->resource = wl_resource_create(wl_client, &shoyu_shell_output_interface, version, id);
  if (shell_output == NULL) {
  ...
  wl_resource_set_implementation(shell_output->resource, &shoyu_shell_output_impl, shell_output, shoyu_shell_output_resource_destroy);
  wlr_addon_init(&shell_output->addon, &wlr_output->addons, shell, &shoyu_shell_output_addon_impl);
}
