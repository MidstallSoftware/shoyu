...
static GParamSpec *shoyu_output_props[N_PROPERTIES] = {
  NULL,
};
...
static void shoyu_output_frame(struct wl_listener *listener, void *data) {
  ...
  struct wlr_render_pass *pass =
      wlr_output_begin_render_pass(self->wlr_output, &state, NULL, NULL);

  if (self->wlr_surface != NULL) {
    ShoyuSurface *surface =
        shoyu_compositor_get_surface(self->compositor, self->wlr_surface);
    if (surface != NULL) {
...
static void shoyu_output_request_state(struct wl_listener *listener,
                                       void *data) {
  ...
  if (self->wlr_surface != NULL) {
    ...
    if (xdg_toplevel != NULL) {
      wlr_xdg_toplevel_set_size(xdg_toplevel, self->wlr_output->width,
...
static void shoyu_output_finalize(GObject *object) {
  ...
  if (!self->is_invalidated)
    g_clear_pointer(&self->wlr_output, (GDestroyNotify)wlr_output_destroy);
  g_clear_object(&self->compositor);

  G_OBJECT_CLASS(shoyu_output_parent_class)->finalize(object);
}

static void shoyu_output_set_property(GObject *object, guint prop_id,
...
static void shoyu_output_get_property(GObject *object, guint prop_id,
...
static void shoyu_output_class_init(ShoyuOutputClass *class) {
  ...
  shoyu_output_props[PROP_SURFACE] = g_param_spec_pointer(
      ...
      "The surface which is to be rendered onto the output.", G_PARAM_READABLE);
   ...
   */
  shoyu_output_sigs[SIG_DESTROY] =
      g_signal_new("destroy", SHOYU_TYPE_OUTPUT, G_SIGNAL_RUN_LAST, 0, NULL,
   ...
   */
  shoyu_output_sigs[SIG_REALIZED] =
      g_signal_new("realized", SHOYU_TYPE_OUTPUT, G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(ShoyuOutputClass, realized), NULL, NULL,
   ...
   * @output: a #ShoyuOutput
   */
  shoyu_output_sigs[SIG_UNREALIZED] =
      g_signal_new("unrealized", SHOYU_TYPE_OUTPUT, G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(ShoyuOutputClass, unrealized), NULL, NULL,
                   NULL, G_TYPE_NONE, 0);
}
...
ShoyuOutput *shoyu_output_new(ShoyuCompositor *compositor) {
  return SHOYU_OUTPUT(
      g_object_new(SHOYU_TYPE_OUTPUT, "compositor", compositor, NULL));
}
...
void shoyu_output_realize(ShoyuOutput *self, struct wlr_output *wlr_output) {
  ...
  self->wlr_output_layout_output =
      wlr_output_layout_add_auto(self->compositor->output_layout, wlr_output);
...
void shoyu_output_set_surface(ShoyuOutput *self,
