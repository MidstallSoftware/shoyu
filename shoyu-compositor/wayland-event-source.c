...
static gboolean shoyu_wayland_event_source_prepare(GSource *source,
                                                   int *timeout) {
  ...
  *timeout = -1;
  ...
  if (self->wl_display != NULL)
    wl_display_flush_clients(self->wl_display);

  return FALSE;
}

static gboolean shoyu_wayland_event_source_dispatch(GSource *source,
...
GSource *shoyu_wayland_event_source_new(struct wl_display *wl_display,
  ...
  GSource *source = g_source_new(&shoyu_wayland_event_source_funcs,
                                 sizeof(ShoyuWaylandEventSource));
