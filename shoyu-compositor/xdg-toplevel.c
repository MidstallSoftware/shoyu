...
static GParamSpec *shoyu_xdg_toplevel_props[N_PROPERTIES] = {
  NULL,
};
...
static void shoyu_xdg_toplevel_destroy(struct wl_listener *listener,
                                       void *data) {
...
static void shoyu_xdg_toplevel_set_property(GObject *object, guint prop_id,
                                            const GValue *value,
                                            GParamSpec *pspec) {
  ShoyuXdgToplevel *self = SHOYU_XDG_TOPLEVEL(object);
...
static void shoyu_xdg_toplevel_get_property(GObject *object, guint prop_id,
...
static void shoyu_xdg_toplevel_class_init(ShoyuXdgToplevelClass *class) {
   ...
   */
  shoyu_xdg_toplevel_sigs[SIG_DESTROY] =
      g_signal_new("destroy", SHOYU_TYPE_XDG_TOPLEVEL, G_SIGNAL_RUN_LAST, 0,
   ...
   */
  shoyu_xdg_toplevel_sigs[SIG_REALIZED] =
      g_signal_new("realized", SHOYU_TYPE_XDG_TOPLEVEL, G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(ShoyuXdgToplevelClass, realized), NULL, NULL,
   ...
   * @output: a #ShoyuXdgToplevel
   */
  shoyu_xdg_toplevel_sigs[SIG_UNREALIZED] =
      g_signal_new("unrealized", SHOYU_TYPE_XDG_TOPLEVEL, G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(ShoyuXdgToplevelClass, unrealized), NULL,
...
ShoyuXdgToplevel *shoyu_xdg_toplevel_new(ShoyuCompositor *compositor) {
  return SHOYU_XDG_TOPLEVEL(
      g_object_new(SHOYU_TYPE_XDG_TOPLEVEL, "compositor", compositor, NULL));
}
...
void shoyu_xdg_toplevel_realize(ShoyuXdgToplevel *self,
  ...
  g_signal_emit(self, shoyu_xdg_toplevel_sigs[SIG_REALIZED], 0,
                wlr_xdg_toplevel);

  if (!shoyu_compositor_is_xdg_toplevel_claimed(self->compositor,
                                                wlr_xdg_toplevel) &&
      self->compositor->shell->resource != NULL) {
...
void shoyu_shell_xdg_toplevel_bind_shell(ShoyuXdgToplevel *self) {
  shoyu_shell_toplevel_create(self->compositor->shell->client,
                              self->compositor->shell->resource,
                              self->wlr_xdg_toplevel);
}

void shoyu_shell_xdg_toplevel_unbind_shell(ShoyuXdgToplevel *self) {
  if (self->wlr_xdg_toplevel != NULL) {
    shoyu_shell_toplevel_delete(self->compositor->shell,
                                self->wlr_xdg_toplevel);
  }
}
