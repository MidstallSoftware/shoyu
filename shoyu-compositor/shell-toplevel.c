...
static void shoyu_shell_toplevel_resource_destroy(struct wl_resource* resource) {
...
void shoyu_shell_toplevel_create(struct wl_client* wl_client, struct wl_resource* shell_resource, struct wlr_xdg_toplevel* xdg_toplevel) {
  ...
  if (wlr_addon_find(&xdg_toplevel->base->surface->addons, shell, &shoyu_shell_toplevel_addon_impl) != NULL) {
    wl_resource_post_error(shell_resource, SHOYU_SHELL_ERROR_TOPLEVEL_ALREADY_CONSTRUCTED, "shoyu_shell_toplevel already constructed for this xdg_toplevel");
    return;
  }
  ...
  shell_toplevel->resource = wl_resource_create(wl_client, &shoyu_shell_toplevel_interface, version, 0);
  if (shell_toplevel == NULL) {
  ...
  wl_resource_set_implementation(shell_toplevel->resource, &shoyu_shell_toplevel_impl, shell_toplevel, shoyu_shell_toplevel_resource_destroy);
  wlr_addon_init(&shell_toplevel->addon, &xdg_toplevel->base->surface->addons, shell, &shoyu_shell_toplevel_addon_impl);
...
void shoyu_shell_toplevel_delete(ShoyuShell* shell, struct wlr_xdg_toplevel* xdg_toplevel) {
  struct wlr_addon* addon = wlr_addon_find(&xdg_toplevel->base->surface->addons, shell, &shoyu_shell_toplevel_addon_impl);
