...
static void shoyu_shell_toplevel_added(void *data,
...
static void shoyu_shell_gtk_wl_registry_global(void *data,
...
static void shoyu_shell_gtk_wl_registry_global_remove(
    void *data, struct wl_registry *wl_registry, uint32_t id) {
  // GdkDisplay* display = GDK_DISPLAY(data);
  // ShoyuShellGtkDisplay* self = g_object_get_data(G_OBJECT(display),
  // SHOYU_SHELL_GTK_DISPLAY_KEY);
}
...
gboolean shoyu_shell_gtk_init_display(GdkDisplay *display) {
  if (g_object_get_data(G_OBJECT(display), SHOYU_SHELL_GTK_DISPLAY_KEY) !=
      NULL) {
    return TRUE;
  }
  ...
  wl_registry_add_listener(wl_registry, &shoyu_shell_gtk_wl_registry_listener,
                           display);
